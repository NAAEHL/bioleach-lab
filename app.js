...
async function dbDeleteUpload(id) {
  const { error } = await sb.from("icp_uploads").delete().eq("id", id);
  if (error) throw new Error(error.message);
}
...
function ICPAnalyser({ experiments, onSaveRun, allUploads, onSaveUpload, onDeleteUpload }) {
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {allUploads.slice(0,10).map(u => (
              <div key={u.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 12px", background:C.surfaceHigh, borderRadius:6, border:`1px solid ${C.border}` }}>
                <div onClick={()=>{ setParsed({samples:u.samples,elements:u.elements}); const dfs={}; u.samples.forEach(s=>{dfs[s.name]=1;}); setDfMap(dfs); setSelected(u.samples[0]?.name); setFileName(u.fileName||u.file_name); }} style={{ flex:1, cursor:"pointer" }}>
                  <span style={{ fontSize:12, fontWeight:600, color:C.text }}>{u.fileName||u.file_name}</span>
                  <span style={{ fontSize:10, color:C.muted, marginLeft:10 }}>{u.samples?.length} samples · {new Date(u.uploadedAt||u.uploaded_at).toLocaleDateString()}</span>
                </div>
                <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                  <span style={{ fontSize:10, color:C.accent, cursor:"pointer" }} onClick={()=>{ setParsed({samples:u.samples,elements:u.elements}); const dfs={}; u.samples.forEach(s=>{dfs[s.name]=1;}); setDfMap(dfs); setSelected(u.samples[0]?.name); setFileName(u.fileName||u.file_name); }}>↩ Reload</span>
                  <button onClick={e=>{ e.stopPropagation(); if(onDeleteUpload) onDeleteUpload(u.id); }} style={{ background:"none", border:"none", color:C.red, cursor:"pointer", fontSize:14, padding:"0 4px" }}>✕</button>
                </div>
              </div>
            ))}
          </div>
...
function App() {
  ...
  async function saveUpload(parsed, fileName) {
    ...
    const updated = [upload, ...allUploads].slice(0, 50);
  ...
  async function deleteUpload(id) {
    const updated = allUploads.filter(u => u.id !== id);
    setAllUploads(updated); lsSave("icpUploads", updated);
    try { await dbDeleteUpload(id); } catch(e) { console.warn("Delete upload error:", e); }
  }
  ...
  return (
    <div style={{ fontFamily:"'IBM Plex Mono','Fira Code',monospace", background:C.bg, minHeight:"100vh", color:C.text }}>
      ...
      <div style={{ padding:"28px 32px", maxWidth:1200 }}>
        ...
        {tab==="analyser"  && <ICPAnalyser experiments={experiments} onSaveRun={saveRun} allUploads={allUploads} onSaveUpload={saveUpload} onDeleteUpload={deleteUpload} />}
