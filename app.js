...
function parseICPData(raw) {
  // PerkinElmer exports sometimes paste as one long line with 
 separators
  // Normalise: try 
 first, then 
, then detect repeated "Sample Id" pattern
  // Use replace(\r) not trim() to preserve leading tabs (PerkinElmer col alignment)
  let lines = raw.split(/\r?\n/).map(l => l.replace(/\r$/,"")).filter(l => l.trim().length > 0);

  // If we only got 1-2 lines, the data might be space-separated records on one line
  // PerkinElmer rows always start with a number then tab then sample name
  // Split on pattern: space + digit(s) + tab  OR  space + whitespace + "Sample Id"
  if (lines.length < 3) {
    // Try splitting on \r
    lines = raw.split(/\r/).map(l => l.replace(/\r$/,"")).filter(l => l.trim().length > 0);
  }
  if (lines.length < 3) {
    // PerkinElmer specific: rows separated by spaces, each row starts with digits or spaces+Sample
    // Split on: (space)(digit+)(\t) pattern — each new row starts with row number
    const normalized = raw.replace(/ (\d{1,3}\t)/g, "\n$1").replace(/ (\s*Sample Id)/g, "\nSample Id");
