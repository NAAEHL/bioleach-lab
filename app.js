...
function parseICPData(raw) {
  // Step 1: merge multi-line cells (Excel wraps cell content with internal newlines)
  // Pattern: a line ending mid-cell (no tab after the last quote) continues on next line
  // We join lines that are inside quoted cells
  ...
  for (const line of rawLines) {
    ...
    // Count unescaped quotes — if even number, cell is complete
  ...
  const lines = merged.filter(l => l.replace(/\t/g,"").trim().length > 0);
  if (lines.length < 2) return null;

  const sep = "\t";
  const clean = c => c.trim().replace(/^"|"$/g,"").replace(/[\n\r]/g," ").replace(/\s+/g," ");
  const SKIP = ["blank","std","calibrat","mili","qc","wire","hno","miliq","milli","impu","calibration"];

  // Pass 1: collect all header blocks
  const headerBlocks = [];
  for (let r = 0; r < lines.length; r++) {
    const cols = lines[r].split(sep).map(clean);
    const sidIdx = cols.findIndex(h => /^sample\s*id$/i.test(h));
    if (sidIdx === -1) continue;
    const elementCols = {};
    cols.forEach((h, i) => {
      const m = h.match(/^([A-Z][a-z]?)\s+[\d.]+/);
      if (m) elementCols[m[1]] = i;
    });
    if (Object.keys(elementCols).length > 0) {
      headerBlocks.push({ lineIdx: r, sidIdx, elementCols });
    }
  }
  if (headerBlocks.length === 0) return null;

  // Pass 2: extract sample rows
  const allElements = {};
  headerBlocks.forEach(h => Object.keys(h.elementCols).forEach(el => allElements[el] = true));
  const samples = [];
  ...
  for (let r = 0; r < lines.length; r++) {
    ...
    if (!hb) continue;
    const cols = lines[r].split(sep).map(clean);
    const name = (cols[hb.sidIdx] || "").trim();
    if (!name) continue;
    if (/^sample\s*id$/i.test(name)) continue;
  ...
  return samples.length ? { samples, elements: Object.keys(allElements) } : null;
}
  ...
  return (
    <div>
      <div style={S.card}>
            ...
            <input type="file" accept=".xls,.xlsx" style={{ display:"none" }} onChange={e => {
              ...
              reader.onload = evt => {
                try {
                  ...
                  const sheetName = wb.SheetNames.find(n => /sample.?unit/i.test(n) || /conc/i.test(n)) || wb.SheetNames[0];
                  const ws = wb.Sheets[sheetName];
                  const tsv = XLSX.utils.sheet_to_csv(ws, { FS:"\t", defval:"" });
                  setRaw(tsv);
                  setError("");
        ...
        <div style={{ fontSize:11, color:C.muted, marginBottom:8 }}>
          <span style={{ color:C.textDim, fontWeight:600 }}>Option B — Paste from Excel</span> (Ctrl+A → Ctrl+C on the "Conc. in Sample Units" sheet)
        </div>
