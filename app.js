...
const THEMES = {
  ...
  light: {
    bg: "#eef3f8", surface: "#ffffff", surfaceHigh: "#e3ebf3",
    border: "#c2d2e0", accent: "#1a5fa8", accentGlow: "rgba(26,95,168,0.10)",
    green: "#187a3f", yellow: "#8a5d05", red: "#ad2418",
    muted: "#2c3644", text: "#0a0e14", textDim: "#1c2430",
  },
};
let C = { ...THEMES.dark }; // mutated in place by App() at every render so all getS()/getColors() reads stay in sync
function getColors(theme) { return THEMES[theme] || THEMES.dark; }

const REE = ["Ce","Dy","Er","Eu","Gd","Ho","La","Lu","Nd","Pr","Sc","Sm","Tb","Th","Tm","U","Y","Yb"];
const ICP_MIN = 1, ICP_MAX = 50;       // machine detection range (alerts in ICP Analysis)
const TARGET_MIN = 0.5, TARGET_MID = 2, TARGET_MAX = 5; // recommended target: sweet spot ~2 ppm (centre of 0.5-5 ppm linear range)
...
function getStatusColor() { return { "Completed":C.green, "In progress":C.accent, "Pending":C.yellow, "Abandoned":C.red }; }
const EL_GROUPS = [
...
const targetStatus = v => (v==null||isNaN(v)) ? "unknown" : v>TARGET_MAX ? "high" : v<TARGET_MIN ? "low" : "ok";
...
function recommendDF(est) {
  ...
  // Target midpoint of recommended detection window (~5 ppm), not the machine upper limit (50 ppm)
  if (max <= TARGET_MAX) return 1;
  return Math.ceil(max / TARGET_MID);
}
          ...
          <table style={S.table}>
            ...
            <tbody>
              {Object.entries(result.est).map(([el,c]) => {
                const proj=result.projected[el]; const s=targetStatus(proj);
                return (
          ...
          {[
            ...
            <div key={g.label} style={{ ...S.card, border:`1px solid ${g.color}44`, marginBottom:16 }}>
              ...
              <table style={S.table}>
                ...
                <tbody>
                  {g.els.map(el=>{
                    const c=result.est[el]; const proj=g.proj[el]; const s=targetStatus(proj);
                    return (
                      <tr key={el} style={{ background:s!=="ok"?statusBg(s):"transparent" }}>
                        <td style={{ ...S.td, fontWeight:600, color:REE.includes(el)?C.accent:C.text }}>
                          {el}{REE.includes(el)&&<span style={{fontSize:9,color:C.accent,marginLeft:4}}>REE</span>}
...
function ICPAnalyser({ experiments, onSaveRun, allUploads, onSaveUpload }) {
  ...
  return (
    <div>
      <div style={S.card}>
        <div style={S.cardTitle}>Import ICP data — "Conc. in Sample Units" sheet</div>

        <div style={{ marginBottom:14 }}>
          <div style={{ fontSize:11, color:C.muted, marginBottom:8 }}>
            <span style={{ color:C.accent, fontWeight:600 }}>Option A — Upload file directly</span> (.xls or .xlsx from PerkinElmer)
          </div>
          <label style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"8px 16px", borderRadius:6, border:`1px solid ${C.accent}`, background:C.accentGlow, color:C.accent, fontSize:12, fontWeight:600, cursor:"pointer", textTransform:"uppercase", letterSpacing:"0.05em" }}>
            📂 Upload XLS / XLSX
            <input type="file" accept=".xls,.xlsx" style={{ display:"none" }} onChange={e => {
              ...
              reader.onload = evt => {
                try {
                  ...
                  const sheetName = wb.SheetNames.find(n => /sample.*unit/i.test(n) && !/rsd/i.test(n) && !/calib/i.test(n))
                    || wb.SheetNames.find(n => /sample/i.test(n) && !/rsd/i.test(n))
                    || wb.SheetNames[0];
                  const ws = wb.Sheets[sheetName];
                  const range = XLSX.utils.decode_range(ws["!ref"] || "A1");
                  const SKIPW = ["blank","std","calibrat","mili","qc","wire","hno","miliq","milli","impu","calibration"];
                  let headerBlocks = [], allElements = {}, samples = [];
                  for (let r = range.s.r; r <= range.e.r; r++) {
                    ...
                    const sidIdx = row.findIndex(h => /^sample\s*id$/i.test(h));
                    if (sidIdx !== -1) {
                      const ec = {};
                      row.forEach((h,i) => { const m=h.match(/^([A-Z][a-z]?)\s+[\d.]+/); if(m){ec[m[1]]=i;allElements[m[1]]=true;} });
                      if (Object.keys(ec).length>0) headerBlocks.push({lineIdx:r,sidIdx,elementCols:ec});
                      continue;
                    }
                    const hb = [...headerBlocks].reverse().find(h=>h.lineIdx<r);
                    if (!hb) continue;
                    const name = (row[hb.sidIdx]||"").trim();
                    if (!name||/^sample\s*id$/i.test(name)||SKIPW.some(s=>name.toLowerCase().includes(s))||/^\d+$/.test(name)) continue;
                    const data = {};
                    for (const [el,ci] of Object.entries(hb.elementCols)) {
                      const v=(row[ci]||"").trim();
                      data[el]=(v&&v!=="-"&&!isNaN(parseFloat(v)))?parseFloat(v):null;
                    }
                    if (!Object.values(data).some(v=>v!==null)) continue;
                    if (!samples.find(s=>s.name===name)) samples.push({name,data});
                  }
                  if (samples.length===0){setError("No samples found in: "+sheetName);return;}
                  const result={samples,elements:Object.keys(allElements)};
                  setParsed(result);
                  const dfs={}; samples.forEach(s=>{dfs[s.name]=1;});
                  setDfMap(dfs); setSelected(samples[0]?.name);
                  setFileName(file.name+" — "+sheetName);
                  setError("");
                  if (onSaveUpload) onSaveUpload(result, file.name+" — "+sheetName);
                } catch(err) {
          ...
          {fileName && <span style={{ fontSize:11, color:C.muted, marginLeft:10 }}>{fileName}</span>}
        </div>

        <div style={{ fontSize:11, color:C.muted, marginBottom:8 }}>
          <span style={{ color:C.textDim, fontWeight:600 }}>Option B — Paste from Excel</span> (Ctrl+A → Ctrl+C on the sheet)
        </div>
        <textarea style={{ ...S.textarea, minHeight:80 }} value={raw} onChange={e=>setRaw(e.target.value)} placeholder="Paste here..." />
        ...
        <div style={{ marginTop:12, display:"flex", gap:10, alignItems:"center" }}>
          ...
          {parsed && <button style={S.btn("secondary")} onClick={()=>{setParsed(null);setRaw("");setFileName("");}}>Clear</button>}
        ...
        <div style={S.card}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {allUploads.slice(0,10).map(u => (
              <div key={u.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 12px", background:C.surfaceHigh, borderRadius:6, border:`1px solid ${C.border}` }}>
                <div onClick={()=>{ setParsed({samples:u.samples,elements:u.elements}); const dfs={}; u.samples.forEach(s=>{dfs[s.name]=1;}); setDfMap(dfs); setSelected(u.samples[0]?.name); setFileName(u.fileName); }} style={{ flex:1, cursor:"pointer" }}>
                ...
                <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                  <span style={{ fontSize:10, color:C.accent, cursor:"pointer" }} onClick={()=>{ setParsed({samples:u.samples,elements:u.elements}); const dfs={}; u.samples.forEach(s=>{dfs[s.name]=1;}); setDfMap(dfs); setSelected(u.samples[0]?.name); setFileName(u.fileName); }}>↩ Reload</span>
                  <button onClick={e=>{ e.stopPropagation(); setAllUploads(prev=>{ const updated=prev.filter(x=>x.id!==u.id); lsSave("icpUploads",updated); return updated; }); }} style={{ background:"none", border:"none", color:C.red, cursor:"pointer", fontSize:14, padding:"0 4px" }}>✕</button>
                </div>
              </div>
            ))}
          </div>
...
function ExperimentDashboard({ experiments, setExperiments, allRuns, syncStatus }) {
  ...
  async function removeRun(expId, runId) {
    const updatedExps = experiments.map(e => e.id===expId ? {...e, icpRunIds:(e.icpRunIds||[]).filter(id=>id!==runId)} : e);
    ...
    try { await dbSaveExperiment(updatedExps.find(e=>e.id===expId)); } catch(e) { console.warn("Sync error:", e); }
  }
          ...
          {expRuns.length===0
            ...
            : <table style={S.table}>
                <thead><tr>
                  ...
                  <th style={S.th}>Alerts</th><th style={S.th}></th>
                </tr></thead>
                <tbody>
                  {expRuns.map(run=>{
                    const df=run.df||1;
                    ...
                    return (
                      <tr key={run.id}>
                        ...
                        <td style={S.td}><button onClick={()=>removeRun(exp.id,run.id)} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:14,padding:"0 4px"}}>✕</button></td>
                      </tr>
                    );
                  })}
                </tbody>
...
function App() {
  ...
  const [theme,       setTheme]       = useState(() => lsLoad("theme", "dark"));

  // Mutate the shared C object in place so every component (reading C.xxx at render time) picks up the new theme
  Object.assign(C, getColors(theme));
  const S = getS();
  ...
  async function saveRun(expId, sample, df) {
    // Prevent duplicate: if a run with same name already linked, skip
    const exp = experiments.find(e=>e.id===expId);
    const linkedRunIds = exp?.icpRunIds || [];
    const alreadyLinked = allRuns.find(r => linkedRunIds.includes(r.id) && r.name === sample.name);
  ...
  return (
    <div style={{ fontFamily:"'IBM Plex Mono','Fira Code',monospace", background:C.bg, minHeight:"100vh", color:C.text }}>
      <style>{`
        ::placeholder { color: ${C.muted}; opacity: 0.75; }
      ...
      <div style={{ borderBottom:`1px solid ${C.border}`, padding:"18px 32px", display:"flex", alignItems:"center", gap:14, background:C.surface }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:18 }}>
          <div style={{ fontSize:10, color:C.muted, textAlign:"right", lineHeight:1.8 }}>
          ...
          <button
            ...
            style={{ width:34, height:34, borderRadius:8, border:`1px solid ${C.border}`, background:C.surfaceHigh, color:C.text, fontSize:15, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}
          >
            {theme === "dark" ? "☀️" : "🌙"}
          </button>
        </div>
