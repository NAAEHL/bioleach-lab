...
function parseICPData(raw) {
  ...
  const SKIP  = ["blank","std","calibrat","mili","qc","wire","hno","miliq","milli","impu"];

  // --- Pass 1: collect ALL header blocks (lines containing "Sample Id") ---
  // and build a master map: column_index -> element_symbol
  // We track the LAST header seen before each data row.
  const headerBlocks = []; // [{lineIdx, sidIdx, elementCols:{el->colIdx}}]
  ...
  // --- Pass 2: for each non-header line, find the most recent header above it ---
  const allElements = {};
  headerBlocks.forEach(h => Object.keys(h.elementCols).forEach(el => allElements[el] = true));

  const samples = [];
  for (let r = 0; r < lines.length; r++) {
    // Find applicable header (most recent header block before this line)
    ...
    if (!name) continue;
    if (/^sample\s*id$/i.test(name)) continue; // skip header rows
    if (SKIP.some(s => name.toLowerCase().includes(s))) continue;
    if (/^\d+$/.test(name)) continue; // pure row numbers

    // Must have at least one numeric value in element columns
    const data = {};
