                  ...
                  {expRuns.map(run=>{
                    ...
                    return (
                      <tr key={run.id}>
                        ...
                        <td style={S.td}><button onClick={()=>removeRun(exp.id,run.id)} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:13,padding:"0 4px"}}>✕</button></td>
                      </tr>
                    );
                  })}
                </tbody>
...
function App() {
  ...
  async function saveRun(expId, sample, df) {
    // Prevent duplicate: if a run with same name is already linked to this experiment, skip
