...
function parseICPData(raw) {
  // Split without trim() to preserve leading tabs (PerkinElmer column alignment)
  const lines = raw.split(/\r?\n/).map(l => l.replace(/\r$/,"")).filter(l => l.replace(/\t/g,"").trim().length > 0);
  ...
  // Pass 2: for each data row, use most recent header block above it
  const allElements = {};
